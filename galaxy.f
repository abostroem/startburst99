...
c CHOOSEWR:     SELECTS THE CORRECT MODEL FROM JOHN HILLIER'S SUITE OF
c               WR ATMOSPHERES. 
...
c               TEFF AND LOG (FUTURE IMPLEMENTATION).
...
c               IFA ATMOPSHERES (FUTURE IMPLEMENTATION).
...
c FLUX_HIRES:VECTOR OF DIMENSION 13323 CONTAINING THE SYNTHETIC SPECTRUM
...
c NMOD(2):  NUMBER OF WN (WC) ATMOSPHERES IN HILLIER'S GRID.
...
c           CONTINUUM IS INCLUDED. (FUTURE IMPLEMENTATION).
...
c           IN THE IFA MODEL ATMOSPHERES. (FUTURE IMPLEMENTATION).
...
c
        ...
        open(unit=20,name=file_name,status='old',err=999)
...
c *** THE FILE NAME IS LOCATION DEPENDENT ***
c
        open(unit=9,name='lejeune/wr_beta1.fluxes',status='old')
	do 1 i=1,12
	do 2 j=1,10
	do 3 k=1,1221
...
c *** THE FILE NAME IS LOCATION DEPENDENT ***
c
        open(unit=9,name='lejeune/wr_beta2.fluxes',status='old')
	do 6 i=1,11
	do 7 j=1,8
	do 8 k=1,1221
...
c *** THE FILE NAME IS LOCATION DEPENDENT ***
c
      open(unit=10,name='auxil/schkal.dat',status='old')
      do 499 i=1,445
      read(10,*) bip(i),tip(i)
499   continue
      close(unit=10)
...
c *** THE FILE NAME IS LOCATION DEPENDENT ***
c
        open(unit=15,name='auxil/irfeatures.dat',status='old')
...
c
      ...
      open (unit=11,name=file_name,status='old',err=100)
...
c
      nmod=12
      nmod2=12
      open(unit=49,name=file_wn,status='old')
      open(unit=48,name=file_wc,status='old')
c
c THE WN GRID
c
      do i=1,13
...
c
        ...
        open(unit=30,name=file_name,status='old')
	do 501 i=1,13321,7
...
c
       ...
       open(unit=20,name=file_name,status='old',err=699)
        do 1002 j=1,nmodels
...
c
        ...
        open(unit=21,name=file_name,status='old',err=998)
        do 2002 j=1,nmodels
...
c
      if(iline.eq.1) then
      open(unit=15,name='auxil/sp.dat',status='old')
      do 86 j=1,450
      do 87 k=1,np,5
...
c
      if(iline.eq.2) then
      open(unit=15,name='auxil/sp_low.dat',status='old')
      do 860 j=1,450
      do 870 k=1,np,5
...
c
      if(iline.eq.1) then
      open(unit=15,name='auxil/fuse_high.dat',status='old')
      do 76 j=1,450
      do 77 k=1,np1,5
...
c
      if(iline.eq.2) then
      open(unit=15,name='auxil/fuse_low.dat',status='old')
      do 760 j=1,450
      do 770 k=1,np1,5
...
c
        ...
        open(unit=20,name=file_name,status='old',err=399)
        do 1001 j=1,86
...
c
        ...
        open(unit=21,name=file_name,status='old',err=398)
        do 2001 j=1,86
...
c
	if(tmax.gt.9.99e8) then
		jtime = 1
		write(6,32)
32		format(' TIME STEP CHANGED TO LOGARITHMIC IF ',
     *                'TMAX >= 1 GYR! OTHERWISE THE JOB TAKES ',
     *                'TOO LONG!')		
	endif	
c
...
c
     ...
     *    (35.lt.iz.and.iz.lt.41).or.iz.gt.45) then
		write(6,21)
...
c
        do j=1,Ninterv
c
         if (xmlow.le.xmaslim(j).and.xmaslim(j).le.xmhigh) then
          if (xponent(j).eq.1.) then
           dens(i)=a(j)*(log(xmaslim(j))-log(xmlow)) + 
     *             a(j+1)/(1.-xponent(j+1))*
     *             (xmhigh**(1.-xponent(j+1))-
     *             xmaslim(j)**(1.-xponent(j+1)))
          else if (xponent(j+1).eq.1.) then
           dens(i)=a(j)/(1.-xponent(j))*(xmaslim(j)**(1.-xponent(j))-
     *             xmlow**(1.-xponent(j))) + 
     *             a(j+1)*(log(xmhigh)-log(xmaslim(j+1))) 
          else
c
...
c
		if(j.eq.1) then
           dens(i)=a(j)/(1.-xponent(j))*(xmaslim(j)**(1.-xponent(j))-
     *             xmlow**(1.-xponent(j))) + 
     *             a(j+1)/(1.-xponent(j+1))*(xmhigh**(1.-xponent(j+1))-
     *             xmaslim(j)**(1.-xponent(j+1)))
		else
...
c 
c READS DIFFERENT SETS OF EVOLUTIONARY TRACKS FROM GENEVA 1992-1994 AND PADOVA 
c 1993-1994, AND PADOVA 2000 WHITH MASS LOSS FOR THE TP-AGB STARS.
c ALSO SETS UP THE MASS LIMIT FOR WR FORMATION (XMWR), AS GIVEN BY MAEDER & 
c MEYNET (1994, A&A, 287, 803, Table 1) FOR ALL SETS OF TRACKS.
c
c ****************************************************************************
c
        implicit real*4    (a-h,o-z)
     ...
     *           io9,io10,io11,io12,io13,io14,io15,xmwr,
     *           iwrscale,iwrt,jtime,tvar,tiempo1,tinter
        parameter (ntracks = 50, nptrack = 100)
...
c EXPLODED AS SUPERNOVAE AND HAVE VANISHED FROM THE HRD.
c
      do i=1,nma
         da(i,1)      = 1.e-3
...
c      write(870,9410)
c9410    format('  <Columns>COIND/CaEQ1/CaEQ2/CO162/CO229/Si159'
c     *               ,'</Columns>')
c          
988     continue
c
        write(87,448) time_in,co,ca,ca5,co162,co229,si159
...
c
     ...
     *         freeflux1(22),flx(4200)
     	data freelam1/895.,1085.,1150.,1280.,1310.,1360.,1430.,1480.,
     *         1510.,1580.,1630.,1680.,1740.,1785.,1820.,1875.,2035.,
     *         2235.,2565.,2745.,2895.,3010./
c 
...
c
	if(temp(l).gt.4.4.and.xsurf(l).lt.0.4.and.cmass(l).ge.xmwr) then
           call hillier(l,teff,radius)
c
c PERFORM AN APPROXIMATE NORMALIZATION OF THE CONTINUUM FOR THE HILLIER
c FLUXES.
c
	do 4560 jj=1,22
...
c WNL STARS (HEII4686 IS REFERENCE):
      ...
      if (zmetal.ge.0.018) then
         wrfluxes(2,2) = 0.616 * wrfluxes(2,4)	
      else
         wrfluxes(2,2) = 0.244 * wrfluxes(2,4)	
      endif
...
c
      get_wrtype = 11
c 
      if (xh.lt.0.4.and.tstar.gt.4.d0) then
         if (xh.le.0.001) then
            if (xc.gt.xn) then
c WC STARS    
...
c ************************************************************************
c
        subroutine renorm_ifa(teff)
c
c  ROUTINE TO RESCALE THE IFA ATMOSPHERES TO TEFF WHEN THE NEAREST
c  ATMOSPHERE IS PICKED.
c
c ************************************************************************
c
     ...
     *         flam1_ifa_c(4200,86),flam_ifa_c(4200)
        dimension fla_temp1(4200),fla_temp2(4200)

        kmax = 4200
c
c RENORMALIZE ATMOSPHERES OF THE WM-BASIC GRID
c
 
          do k=1,4200
            fla_temp1(k)=flam_ifa_l(k)
            fla_temp2(k)=flam_ifa_c(k)
          enddo
c 	  
 	  call total_flux(wave_ifa,fla_temp2,kmax,teff,xinte)
          do k=1,4200
            flam_ifa_c(k)=fla_temp2(k)/xinte
            flam_ifa_l(k)=fla_temp1(k)/xinte
          enddo
c
        return
        end
...
c
c SELECTS THE CORRECT HILLIER WR MODEL. WRITTEN BY RICHARD NORRIS (UCL).
c
c *****************************************************************************
c
        implicit real*4    (a-h,o-z)
...
c
     ...
     *         ysurf(npgrid),xc12s(npgrid),xn14s(npgrid),xo16s(npgrid),
     *         bmdot(npgrid),grav(npgrid),cmass(npgrid),dens(npgrid),
     *         flam_hires_l(13323),flux_hires_l(13323),
     *         toflux_hires_l(13323),toflux_hires_c(13323),
     *         flux_hires_c(13323),wave_hires(13323),
...
c
     ...
     *       12.566*radius*radius/1.e20*flam_hires_c(m)*dens(l)
           stflux_hires_l(m) = stflux_hires_l(m) + flux_hires_l(m)
           stflux_hires_c(m) = stflux_hires_c(m) + flux_hires_c(m)
20      continue
10	continue
c      
c CALCULATE THE NEBULAR CONTINUUM AND ADD TO THE STELLAR CONTINUUM.
...
c USING CALIBRATION OF DIAZ ET AL. (MNRAS, 239, 325 [1989]).  
c 
c ****************************************************************************
c
	implicit real*4    (a-h,o-z)      
...
c
	insabund=irsg
c
c THIRD: RESET A TEMPORARY VARIABLE TTEMP AS TEFF.  WE MUST NOT CHANGE TEFF,
c BUT WE CAN CHANGE TTEMP!
c 
	ttemp=teff
c
